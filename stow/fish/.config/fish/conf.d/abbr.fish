abbr md "mkdir"
abbr rd "rmdir"
abbr sd "cd (fd -t dir | fzf)"
abbr groh "git reset origin/(__git.current_branch) --hard"
abbr jab "jj abandon"
abbr je "jj edit"
abbr jst "jj st"
abbr jsp "jj split -i"
abbr jdm "jj diff --from (__git.default_branch)"
abbr jf "jj git fetch"
abbr jfa "jj git fetch --all-remotes"
abbr jp "jj git push"
abbr jpa "jj git push --all"
abbr jpan "jj git push --allow-new"
abbr jpb "jj git push --bookmark"
abbr jpt "jj git push --tracked"
abbr jn "jj new"
abbr jc "jj describe"
abbr jsh "jj show"
abbr jb "jj bookmark"
abbr jba "jj bookmark advance"
abbr jbc "jj bookmark create"
abbr jbd "jj bookmark delete"
abbr jbf "jj bookmark forget"
abbr jbl "jj bookmark list"
abbr jbla "jj bookmark list --all"
abbr jbt "jj bookmark track"
abbr jbm "jj bookmark move"
abbr jrb "jj rebase"
abbr jrba "jj rebase -s 'roots(trunk()..) & mutable()' -d (__git.default_branch) --skip-emptied"
abbr jrbm "jj rebase -b @ -d (__git.default_branch) --skip-emptied"
abbr jrs "jj restore"
abbr jdu "jj duplicate"
abbr jsq "jj squash"
abbr jlm "jj log -r ::(__git.default_branch)"
abbr jla "jj log -r 'all()'"
abbr jls "jj log -r '::main & ~description(substring:\"chore(deps)\")'"
abbr jt "jj tag list"
abbr jcl "jj git clone --colocate"
